...
// Dibuja tres veces lo mismo y sale un cuadrado.
void draw_pixel(int x, int y, int color) {
    // Dibujamos un pixel en blanco en la posicion 0.
    // (y*ancho + x)*3
	char *curpos = screenData->framebuffer;
	unsigned data = getPixelDataByPosition(x, y);
	curpos[data] = color & 255;
	curpos[data + 1] = (color >> 8) & 255;
	curpos[data + 2] = (color >> 16) & 255;
}